...
fn main() {
    ...
    let img = image::RgbaImage::from_raw(256, 256, preview_rgba)
        .expect("Failed to create preview image");
...
fn scale_sprite_to_rgba(pixels: &[[u8; 20]; 28], src_w: u32, src_h: u32, target_size: u32) -> Vec<u8> {
    ...
    for y in 0..target_size {
        for x in 0..target_size {
            let color = if x >= offset_x && x < offset_x + scaled_w 
