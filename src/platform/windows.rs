...
use super::{
    ...
    FrameResult, KeyCode, KeyEvent, LogLevel,
};
        ...
        match msg {
            ...
            WM_KEYDOWN | WM_SYSKEYDOWN => {
                ...
                if let Some(keycode) = vk_to_keycode(vk) {
                    let event = KeyEvent { key: keycode, pressed: true };
                    if let Some(state) = GAME_STATE.as_mut() {
            ...
            WM_KEYUP | WM_SYSKEYUP => {
                let vk = wparam as u16;
                if let Some(keycode) = vk_to_keycode(vk) {
                    let event = KeyEvent { key: keycode, pressed: false };
                    if let Some(state) = GAME_STATE.as_mut() {
                        state.handle_key_event(&event);
                    }
                }
...
unsafe fn init_gpu_renderer(hwnd: HWND) -> bool {
    ...
    let adapter = match futures::executor::block_on(instance.request_adapter(&wgpu::RequestAdapterOptions {
    ...
    println!("[GPU] Device: {} ({:?})", adapter_info.name, adapter_info.device_type);
    println!("[GPU] GPU renderer initialized (Surface: {:?})", surface_format);
    true
}
...
pub fn run_game() -> std::result::Result<(), Box<dyn std::error::Error>> {
    unsafe {
        ...
        let hicon = if hicon.is_null() { LoadIconW(null_mut(), IDI_APPLICATION) } else { hicon };
