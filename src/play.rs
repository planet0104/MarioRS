...
use crate::{
    ...
    mpal256,
    music::MusicPlayer,
    palettes::{Palettes, PE_BLACK_WHITE, PE_EGA_MODE, PE_NO_EFFECT},
    players::Players,
...
impl Play {
    ...
    fn get_current_opt(&self, buffers: &Buffers) -> WorldOptions {
        self.current_opt.clone().unwrap_or_else(|| buffers.options.clone())
    }
    ...
    fn frame_update_inner(
        ...
        // 只有不在暂停状态才检测ESC退出游戏
        if self.phase != PlayPhase::Paused {
        ...
        match self.phase {
            ...
            PlayPhase::InitVga => {
                ...
                // 锁定游戏调色板对象，防止渲染初始帧时写入VGA导致闪烁
                palette.lock_pal();
                
                // 初始化调色板
                let p = mpal256::mpal256_palette();
                palette.new_palette(p);
                palette.read_palette(vga, p);
                ...
                let (map1_data, opt1, map2_data, opt2, world_number) = match self.current_level_index {
                    0 => {
                        use crate::worlds::level_1::{LEVEL_1A_MAP, LEVEL_1B_MAP, OPTIONS_1A, OPTIONS_1B, OPT_1A};
                        if turbo {
                            (LEVEL_1A_MAP, OPT_1A.clone(), LEVEL_1B_MAP, OPTIONS_1B.clone(), "x-1")
                        } else {
                            (LEVEL_1A_MAP, OPTIONS_1A.clone(), LEVEL_1B_MAP, OPTIONS_1B.clone(), "x-1")
                        }
                    }
                    1 => {
                        use crate::worlds::level_2::{LEVEL_2A_MAP, LEVEL_2B_MAP, Level2Options};
                        if turbo {
                            (LEVEL_2A_MAP, Level2Options::opt_2a(), LEVEL_2B_MAP, Level2Options::options_2b(), "x-2")
                        } else {
                            (LEVEL_2A_MAP, Level2Options::options_2a(), LEVEL_2B_MAP, Level2Options::options_2b(), "x-2")
                        }
                    }
                    2 => {
                        use crate::worlds::level_3::{LEVEL_3A_MAP, LEVEL_3B_MAP, Level3Options};
                        if turbo {
                            (LEVEL_3A_MAP, Level3Options::opt_3a(), LEVEL_3B_MAP, Level3Options::options_3b(), "x-3")
                        } else {
                            (LEVEL_3A_MAP, Level3Options::options_3a(), LEVEL_3B_MAP, Level3Options::options_3b(), "x-3")
                        }
                    }
                    3 => {
                        ...
                        if turbo {
                            (LEVEL_5A_MAP, Level5Options::opt_5a(), LEVEL_5B_MAP, Level5Options::options_5b(), "x-4")
                        } else {
                            (LEVEL_5A_MAP, Level5Options::options_5a(), LEVEL_5B_MAP, Level5Options::options_5b(), "x-4")
                        }
                    }
                    4 => {
                        ...
                        if turbo {
                            (LEVEL_6A_MAP, Level6Options::opt_6a(), LEVEL_6B_MAP, Level6Options::options_6b(), "x-5")
                        } else {
                            (LEVEL_6A_MAP, Level6Options::options_6a(), LEVEL_6B_MAP, Level6Options::options_6b(), "x-5")
                        }
                    }
                    5 | _ => {
                        ...
                        if turbo {
                            (LEVEL_4A_MAP, Level4Options::opt_4a(), LEVEL_4B_MAP, Level4Options::options_4b(), "x-6")
                        } else {
                            (LEVEL_4A_MAP, Level4Options::options_4a(), LEVEL_4B_MAP, Level4Options::options_4b(), "x-6")
                        }
                    }
                };
                ...
                // 初始化玩家
                players.init_player(opt1.init_x as i32, opt1.init_y as i32, cur_player, buffers, enemies);
            ...
            PlayPhase::BuildLevel => {
                ...
                figures.init_walls(
                    current_opt.wall_type1, current_opt.wall_type2, current_opt.wall_type3,
                    sprites, current_opt,
                );
            ...
            PlayPhase::Restart => {
                ...
                if is_pipe_restart && players.in_pipe {
                    // 检测管道出口方向并触发相应的出管道动画
                    use crate::buffers::{DM_UP_OUT_OF_PIPE, DM_DOWN_OUT_OF_PIPE};
            ...
            PlayPhase::RenderInitFrames => {
                ...
                // 设置调色板（天空/远景/草地）
                figures.set_sky_palette(palette, &current_opt);
                backgr.draw_pal_backgr(palette, vga, Some(&current_opt));
                palette.init_grass(vga, &current_opt);
                
                // 开始淡入 - 同时解锁VGA和游戏调色板
                vga.unlock_pal();
                palette.unlock_pal();
                palette.start_fade_up_steps(64);
            ...
            PlayPhase::FadingUp => {
                if palette.is_fading() {
                    ...
                    if current_opt.sky_type == 8 && current_opt.backgr_type == 4 {
                        ...
                        palette.out_palette(0xFD, 17, 10, 10, vga);  // 深红棕色
                        palette.out_palette(0xFE, 11,  5,  5, vga);  // 暗红棕色
                        palette.out_palette(0xFF, 20, 14, 14, vga);  // 中红棕色
                        backgr.brick_palette(0, palette, vga);
                    }
            ...
            PlayPhase::GameLoop => {
                self.game_loop_tick(
                    ...
                    palette, stars, blocks, status_mgr, glitters, tmpobj, sprites,
                    keyboard, joystick, cur_player,
                )
            }
            
            PlayPhase::Paused => {
                self.pause_tick(vga, txt, music, buffers, players, enemies, palette, tmpobj, keyboard)
            }
            ...
            PlayPhase::LevelComplete => {
                PlayResult::LevelComplete
            }
            
            PlayPhase::PlayerDeath => {
    ...
    fn game_loop_tick(
        ...
        if self.waiting {
            ...
            if buffers.passed {
                ...
                if buffers.text_counter > 350 {
                    // 返回 PlayerDeath 让 mario.rs 决定是切换玩家还是返回 Intro
        ...
        backgr.draw_pal_backgr(palette, vga, Some(&current_opt_mut));
        ...
        if players.in_pipe && !buffers.game_done && !self.waiting {
            ...
            glitters.clear_glitter(vga, buffers);
            vga.palette_fade_down_wrapper(64);
            vga.clear_palette();
            vga.lock_pal();
            vga.clear_vga_mem();
            ...
            match players.pipe_code[0] {
                ...
                0xE7 => {
                    // 关卡完成后必须重置in_pipe状态
                    // 否则下一关开始时仍会检测到in_pipe=true，导致无限循环
                    players.in_pipe = false;
                    players.pipe_code = [b' ', b' '];
                    ...
                    if current_scan_code == 15 && !self.pause_tab_mode {
                        ...
                        if let Some(ch) = ascii_char {
                            ...
                            end_pause = self.check_cheat_codes(
                                vga, txt, buffers, players, enemies, tmpobj, keyboard, music, palette,
                            );
                        } else {
    ...
    fn check_cheat_codes(
        ...
        // MONO - 黑白模式
        if cheat == "MONO" {
            palettes.palette_effect = PE_BLACK_WHITE;
            vga.palette.palette_effect = PE_BLACK_WHITE; // 同步到VGA渲染用的调色板
            let pal = palettes.palette.clone();
            palettes.refresh_palette(&pal, vga);
            return true;
        }
        
        // EGAMODE - EGA 16色模式
        if cheat == "EGAMODE" {
            palettes.palette_effect = PE_EGA_MODE;
            vga.palette.palette_effect = PE_EGA_MODE; // 同步到VGA渲染用的调色板
            let pal = palettes.palette.clone();
            palettes.refresh_palette(&pal, vga);
            return true;
        }
        ...
        if cheat == "VGAMODE" || cheat == "COLOR" {
            palettes.palette_effect = PE_NO_EFFECT;
            vga.palette.palette_effect = PE_NO_EFFECT; // 同步到VGA渲染用的调色板
            let pal = palettes.palette.clone();
            palettes.refresh_palette(&pal, vga);
            return true;
        }
